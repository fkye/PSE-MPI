...
namespace {
  ...
  cl::opt<unsigned>
    OpaqueTypeSize("opaque-type-size",
             cl::init(64));

  cl::list<std::string>
    SkipExterals("skip-externals", cl::CommaSeparated);
}
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
  ...
  for (auto &skipExternal: SkipExterals) {
    errs() << skipExternal << '\n';
    skipExternalRegexVec.push_back(std::regex(skipExternal.c_str()));
  }
}
  ...
  for (Module::const_global_iterator i = m->global_begin(),
	...
	size = kmodule->targetData->getTypeStoreSize(ty);
      ...
      if (size) {
        ...
        // Lazy init aa:
        state.mpiManager.updateMemoryObjectForPointer(v, mo);
      }
    } else {
      Type *ty = i->getType()->getElementType();
...
void Executor::executeCall(ExecutionState &state, 
  ...
  if (f && f->isDeclaration()) {
    ...
    case Intrinsic::not_intrinsic:
      ...
      callExternalFunction(state, ki, f, arguments, shadowMetaMOs);
      break;
    ...
    case Intrinsic::vastart:  {
      ...
      if (WordSize == Expr::Int32) {
        executeMemoryOperation(state, true, arguments[0], 
                               sf.varargs->getBaseExpr(), 0, nullptr);
      } else {
        ...
        executeMemoryOperation(state, true, arguments[0],
                               ConstantExpr::create(48, 32), 0, nullptr); // gp_offset
        executeMemoryOperation(state, true,
                               AddExpr::create(arguments[0], 
                                               ConstantExpr::create(4, 64)),
                               ConstantExpr::create(304, 32), 0, nullptr); // fp_offset
        executeMemoryOperation(state, true,
                               AddExpr::create(arguments[0], 
                                               ConstantExpr::create(8, 64)),
                               sf.varargs->getBaseExpr(), 0, nullptr); // overflow_arg_area
        executeMemoryOperation(state, true,
                               AddExpr::create(arguments[0], 
                                               ConstantExpr::create(16, 64)),
                               ConstantExpr::create(0, 64), 0, nullptr); // reg_save_area
      }
      break;
    }
    ...
    state.mpiManager.transferToBasicBlock(state.pc->inst->getParent());
    ...
    if (MPIManager::lazyInitEnabled) {
      for (unsigned i=0; i<numFormals; ++i)
        getArgumentCell(state, kf, i).shadowMetaMO = shadowMetaMOs[i];
    }
  }
}
...
void Executor::transferToBasicBlock(BasicBlock *dst, BasicBlock *src, 
  ...
  state.mpiManager.transferToBasicBlock(dst);
}
...
//  ki->inst->dump();
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      if (state.mpiManager.handleConditionalBranch(*this, state, cond, bi))
        break;
  ...
  case Instruction::Switch: {
    ...
    if (state.mpiManager.handleSwitch(*this, state, cond, si))
      break;
  ...
  case Instruction::Call: {
    ...
    std::vector<const MemoryObject *> shadowMetaMOs(numArgs);
    if (MPIManager::lazyInitEnabled) {
      // Lazy init pointer-typed arguments (do it here because we may have external calls)
      bool lazySuccess = true;
      unsigned argIdx = 0;
      for (auto it = cs.arg_begin(); it != cs.arg_end(); ++it) {
        ++argIdx;
        auto argType = it->get()->getType();
        auto ptrType = dyn_cast<PointerType>(argType);
        // Skip non-pointer and function pointer arguments
        if (!ptrType || ptrType->getElementType()->isFunctionTy())
          continue;
        auto &cell = eval(ki, argIdx, state);
        bool unbound = state.mpiManager.testMemUnboundAndUnset(*this, state, cell.shadowMetaMO);
        if (unbound) {
          Type *elemType = ptrType->getElementType();
          Expr::Width type = OpaqueTypeSize * 8;
          if (elemType->isSized())
            type = getWidthForLLVMType(elemType);
          auto lazyMO = state.mpiManager.lazyInitMem(*this, state, cell.value, (type + 7) / 8, it->get());
          if (!lazyMO) { // Lazy init failed
            lazySuccess = false;
            break;
          }
        }
      ...
      // shadowMOs and offsets
      for (unsigned j = 0; j < numArgs; ++j) {
        auto &cell = eval(ki, j + 1, state);
        shadowMetaMOs[j] = cell.shadowMetaMO;
      }
    }
    ...
    if (f) {
      ...
      executeCall(state, ki, f, arguments, shadowMetaMOs);
    } else {

      // Lazy init
      auto &fpCell = eval(ki, 0, state);
      ref<Expr> fpExpr = fpCell.value;
      bool unbound = false;
      if (MPIManager::lazyInitEnabled) {
        // Lazy init
        unbound = state.mpiManager.testMemUnbound(*this, state, fpCell.shadowMetaMO);
      }
      if (unbound) {
        // Find candidates
        std::vector<Function *> candidates;
        Type *fpType = cs.getCalledValue()->getType();
        for (auto gf: MPIManager::indirectCalleeCandidates) {
          Type *gfpType = gf->getType();
          if (fpType == gfpType)
            candidates.push_back(gf);
          else if (fpType->getPointerElementType()->isFunctionVarArg()) {
            // Case:
            // void f(void(*a)()) { a(); }
            // Argument a of f becomes vararg in IR
            auto fType = dyn_cast<FunctionType>(fpType->getPointerElementType());
            auto gfType = dyn_cast<FunctionType>(gfpType->getPointerElementType());
            if (fType->getNumParams() == 0 && gfType->getNumParams() == 0 &&
                fType->getReturnType() == gfType->getReturnType())
              candidates.push_back(gf);
          }
        }

        if (candidates.size() == 0) {
          terminateStateOnExecError(state, "invalid function pointer type");
        } else if (candidates.size() == 1) {
          auto gfpExpr = Expr::createPointer((uint64_t)(candidates[0]));
          executeCall(state, ki, candidates[0], arguments, shadowMetaMOs);
        } else {
          ExecutionState *free = &state;
          for (unsigned k = 1u; k < candidates.size(); ++k) {
            auto gfpExpr = Expr::createPointer((uint64_t)(candidates[k]));
            StatePair res = fork(*free, EqExpr::create(fpExpr, gfpExpr), true);
            if (res.first)
              executeCall(*res.first, ki, candidates[k], arguments, shadowMetaMOs);
            free = res.second;
            if (!free)
              break;
          }
          if (free) {
            auto gfpExpr = Expr::createPointer((uint64_t) (candidates[0]));
            executeCall(*free, ki, candidates[0], arguments, shadowMetaMOs);
          }
        }
        ...
        do {
          ...
          if (res.first) {
            ...
            if (legalFunctions.count(addr)) {
              ...
              executeCall(*res.first, ki, f, arguments, shadowMetaMOs);
            } else {
              if (!hasInvalid) {
...
//    bindLocal(ki, state, result);
    ...
    const auto &cell = eval(ki, state.incomingBBIndex, state);
    ref<Expr> result = cell.value;
    bindLocal(ki, state, result);

    if (MPIManager::lazyInitEnabled) {
      // For lazy init
      auto &destCell = getDestCell(state, ki);
      destCell.shadowMetaMO = cell.shadowMetaMO;
    }
  ...
  case Instruction::Load: {
    if (MPIManager::lazyInitEnabled) {
      // Lazy init
      auto &cell = eval(ki, 0, state);

//    errs() << "Load: ";
//    cell.value->dump();
//    errs() << cell.shadowMetaMO << '\n';

      bool unbound = state.mpiManager.testMemUnboundAndUnset(*this, state, cell.shadowMetaMO);
      if (unbound) {
        Expr::Width type = getWidthForLLVMType(ki->inst->getType());
        auto lazyMO = state.mpiManager.lazyInitMem(*this, state, cell.value, (type + 7) / 8,
                                                   cast<LoadInst>(ki->inst)->getPointerOperand());
        if (!lazyMO) // Lazy init failed
          break;
      }
    }
    ...
    executeMemoryOperation(state, false, base, 0, ki, nullptr);
    break;
  }
  case Instruction::Store: {
    if (MPIManager::lazyInitEnabled) {
      // Lazy init
      auto &cell = eval(ki, 1, state);
...
//    errs() << cell.shadowMetaMO << '\n';
      ...
      bool unbound = state.mpiManager.testMemUnboundAndUnset(*this, state, cell.shadowMetaMO);
      if (unbound) {
        Expr::Width type = eval(ki, 0, state).value->getWidth();
        auto lazyMO = state.mpiManager.lazyInitMem(*this, state, cell.value, (type + 7) / 8,
                                                   cast<StoreInst>(ki->inst)->getPointerOperand());
        if (!lazyMO) // Lazy init failed
          break;
      }
    }
    ...
    executeMemoryOperation(state, true, base, value, 0, eval(ki, 0, state).shadowMetaMO);
    break;
  }
...
//    bindLocal(ki, state, base);
    ...
    ref<Expr> offset = ConstantExpr::createPointer(0);
    for (std::vector< std::pair<unsigned, uint64_t> >::iterator
           it = kgepi->indices.begin(), ie = kgepi->indices.end();
         it != ie; ++it) {
      uint64_t elementSize = it->second;
    ...
    if (MPIManager::lazyInitEnabled) {
      // For lazy init, we should assert offset >= 0
      bool offsetMayNonNeg;
      auto offsetNonNeg = SgeExpr::create(offset, ConstantExpr::create(0, offset->getWidth()));
      solver->setTimeout(coreSolverTimeout);
      bool success = solver->mayBeTrue(state,
                                       offsetNonNeg,
                                       offsetMayNonNeg);
      solver->setTimeout(0);
      if (success && offsetMayNonNeg) {
        state.addConstraint(offsetNonNeg);
      } else {
        terminateStateEarly(state, "Negative offset in GEP");
        break;
      }

      // Lazy init
      auto &cell = eval(ki, 0, state);
//    errs() << "GEP SMO: " << cell.shadowMetaMO << '\n';
      bool unbound = state.mpiManager.testMemUnboundAndUnset(*this, state, cell.shadowMetaMO);
      if (unbound) {
//      errs() << "GEP Lazy:";
//      cell.value->dump();
        PointerType *ptrType = dyn_cast<PointerType>(kgepi->inst->getOperand(0)->getType());
        assert(ptrType);
        Type *elemType = ptrType->getElementType();
        Expr::Width type = getWidthForLLVMType(elemType);
        state.mpiManager.lazyInitMem(*this, state, cell.value, (type + 7) / 8,
                                     cast<GetElementPtrInst>(ki->inst)->getPointerOperand(),
                                     offset);
      }
//    errs() << "GEP result: ";
//    state.constraints.simplifyExpr(base)->dump();
    }
...
//    ref<Expr> result = eval(ki, 0, state).value;
    const auto &cell = eval(ki, 0, state);
    ref<Expr> result = cell.value;
    bindLocal(ki, state, result);
...
void Executor::run(ExecutionState &initialState) {
  ...
  // For loop tracking
  initialState.mpiManager.transferToBasicBlock(initialState.pc->inst->getParent());
...
void Executor::callExternalFunction(ExecutionState &state,
                                    ...
                                    std::vector< ref<Expr> > &arguments,
                                    std::vector<const MemoryObject *> &shadowMetaMOs) {
  // Skip user-specified externals
  for (const auto &skipRegex: skipExternalRegexVec) {
    if (regex_match(function->getName().str(), skipRegex)) {
      if (!target->inst->getType()->isVoidTy()) {
        // Return symbolic val
        MemoryObject *mo = memory->allocate((getWidthForLLVMType(target->inst->getType()) + 7) / 8, /*isLocal=*/false,
          /*isGlobal=*/true, /*allocSite=*/nullptr,
          /*alignment=*/8);
        executeMakeSymbolic(state, mo, "skipRet");
        auto result = state.addressSpace.objects.lookup(mo);
        assert(result);
        const ObjectState *os = result->second;
        auto symVal = os->read(0, getWidthForLLVMType(target->inst->getType()));

        bindLocal(target, state, symVal);
      }
      return;
    }
  }
  ...
  if (specialFunctionHandler->handle(state, function, target, arguments, shadowMetaMOs))
    return;
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    if (!mo) {
      ...
      ObjectState *sos = nullptr;
      if (MPIManager::lazyInitEnabled) {
        // Lazy init: allocate shadow memory
        sos = state.mpiManager.allocShadowAndInit(*this, state, mo, false);
      }
      ...
      if (reallocFrom) {
        ...
        if (MPIManager::lazyInitEnabled) {
          // Lazy init: copy shadow meta
          auto smoOld = reallocFrom->getObject()->shadowMO;
          auto result = state.addressSpace.objects.lookup(smoOld);
          assert(result);
          const ObjectState *sosOld = result->second;
          count = std::min(sosOld->size, sos->size);
          for (unsigned i = 0; i < count; i++)
            sos->write(i, sosOld->read8(i));
          state.addressSpace.unbindObject(smoOld);
        }
      ...
      // Lazy init aa:
      state.mpiManager.updateMemoryObjectForPointer(target->inst, mo);
    }
  } else {
    ...
    unsigned mbs = (util::GetTotalMallocUsage() >> 20) + (memory->getUsedDeterministicSize() >> 20);
    if (mbs + (example->getZExtValue() >> 14) > MaxMemory) {
      terminateStateEarly(state, "Symbolic sized malloc exceeds memory limit.");
      return;
    }
...
void Executor::executeMemoryOperation(ExecutionState &state,
                                      ...
                                      const MemoryObject *valShadowMetaMO /* undef if read */) {
  // Intercept memory operation to check if we have a race in MPI buffer
  state.mpiManager.checkBufferRace(*this, state, address, isWrite);
      ...
      if (isWrite) {
        if (os->readOnly) {
          ...
          if (MPIManager::lazyInitEnabled) {
            // For lazy init
            //assert(mo->shadowMO);
            if (valShadowMetaMO && mo->shadowMO) {
              auto shadowMemoryMetaAddress = valShadowMetaMO->getBaseExpr();

              // Write shadow meta address to dest shadow memory
              state.mpiManager.writeShadowMetaAddressToShadowMemory(*this, state, mo->shadowMO, offset,
                                                                    shadowMemoryMetaAddress);

            } else { // Not from loadInst or BitcastInst
              const MemoryObject *destShadowMetaMO =
                  state.mpiManager.getShadowMetaMemoryObject(*this, state, mo->shadowMO, offset);
              if (destShadowMetaMO)
                state.mpiManager.clearShadowMeta(*this, state, destShadowMetaMO);
            }
          }
        }          
      } else {
...
//        errs() << "Load SMO: " << mo->shadowMO << '\n';
        ...
        if (MPIManager::lazyInitEnabled) {
          ...
          const MemoryObject *shadowMetaMO =
              state.mpiManager.getShadowMetaMemoryObject(*this, state, mo->shadowMO, offset);
          if (shadowMetaMO) {
//          auto result2 = state.addressSpace.objects.lookup(shadowMetaMO);
//          assert(result2);
//          const ObjectState *srcShadowMetaOS = result2->second;
//          srcShadowMetaOS->read(0, Expr::Int8)->dump();

            getDestCell(state, target).shadowMetaMO = shadowMetaMO;

//          errs() << "Load dest SMMO: " << shadowMetaMO << '\n';
          }
        }
      }
...
//                            NULL, getAddressInfo(*unbound, address));
      terminateStateEarly(*unbound, "Out of bound pointer.");
    }
  }
}
...
void Executor::executeMakeSymbolic(ExecutionState &state, 
  ...
  if (MPIManager::lazyInitEnabled) {
    ...
    state.mpiManager.allocShadowAndInit(*this, state, mo, true);
  }
...
void Executor::runFunctionAsMain(Function *f,
				 ...
				 char **argv,
				 char **envp) {
  // Lazy init: check if the execution is partial (not from the real main function)
  MPIManager::lazyInitEnabled = (f->begin()->begin()->getMetadata("pse_main") != nullptr);
  if (MPIManager::lazyInitEnabled)
    klee_message("Lazy initialization enabled");
