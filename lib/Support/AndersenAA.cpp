...
AliasAnalysis::AliasResult AndersenAAResult::andersenAlias(const Value *v1, const Value *v2) {
...
AliasAnalysis::AliasResult AndersenAAResult::alias(const AliasAnalysis::Location &l1,
...
bool AndersenAAResult::pointsToConstantMemory(const AliasAnalysis::Location &loc,
                                              bool orLocal) {
...
AliasAnalysis::Location AndersenAAResult::getLocation(const GetElementPtrInst *GI) {
  AAMDNodes AATags;
  GI->getAAMetadata(AATags);

  return llvm::AliasAnalysis::Location(GI->getPointerOperand(),
				       4, AATags);
}

AliasAnalysis::AliasResult AndersenAAResult::alias(const Value *v1,
                                                   const Value *v2) {
  if (!v1->getType()->isPointerTy() || !v2->getType()->isPointerTy())
    return AliasAnalysis::NoAlias;
